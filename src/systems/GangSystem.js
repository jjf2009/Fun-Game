...
export class GangSystem {
  ...
  start() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const m = s.add.image(-40, y, 'gang').setDepth(5);
      const label = s.add.text(-40, y - 26, 'OUTSIDER', {
        fontFamily: 'monospace', fontSize: '11px', color: '#ff6b6b', backgroundColor: '#000000aa', padding: { x: 3, y: 1 },
      }).setOrigin(0.5).setDepth(7);
      const x = Phaser.Math.Between(40, 130);
      s.tweens.add({ targets: m, x, duration: 700 });
  ...
  throwBomb() {
    ...
    const flight = 1150;
    const shadow = s.add.image(tx, ty, 'shadow').setScale(0.3).setAlpha(0.9).setDepth(2);
    s.tweens.add({ targets: shadow, scale: 1, duration: flight });
    const bomb = s.add.image(m.x, m.y, 'bomb').setDepth(8);
    const sx = m.x;
    const sy = m.y;
    s.tweens.addCounter({
  ...
  explode(x, y) {
    ...
    const flash = s.add.circle(x, y, CONFIG.bombRadius, 0xffd166, 0.8).setDepth(8);
    s.tweens.add({ targets: flash, scale: 1.4, alpha: 0, duration: 350, onComplete: () => flash.destroy() });
    for (let i = 0; i < 8; i++) {
      const sp = s.add.image(x, y, 'spark').setDepth(8);
      const a = (i / 8) * Math.PI * 2;
      s.tweens.add({
        targets: sp, x: x + Math.cos(a) * 55, y: y + Math.sin(a) * 55, alpha: 0, duration: 400,
