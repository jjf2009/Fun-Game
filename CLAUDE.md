...
- Modes: `CONFIG.modes.easy/hard`. The mode travels in scene data (`mode`), and `GameScene.modeCfg` holds the active settings. Best scores are kept per mode. Room residents come from `systems/RoomManager.js` (seeded, so both co-op devices draw the same thing).
- Story mode (solo): `data.story` = chapter 1–4. `GameScene.story` is a `StoryDirector` (`src/story/`) that replaces the timer, director and water, and hooks `action`/`hint`/`onCaught`. Pop-up screens (Dialogue, Email) go through `scene.runOverlay(key, data, onDone)`. All story text is in `src/story/chapters.js`. Keep the ragging scenes non-violent (push-ups, singing).
- Leaderboard: `src/leaderboard.js` (Firestore lite, lazy-loaded; collections `scores_easy/hard/story`, rules in `firestore.rules`). It's off until `CONFIG.leaderboard.firebase` is set. `?leaderboard=mock` uses localStorage for testing. End screens add the button with `addSubmitButton()` from `src/ui/submitScore.js`, and HTML inputs go through `src/ui/textBox.js`.
